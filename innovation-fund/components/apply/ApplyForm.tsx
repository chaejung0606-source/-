...
import { APPLICATION_TYPE_LABELS, APPLICATION_PHASE_LABELS, calcSupportTotal, CLUB_PRESIDENT_MONTHLY, MD_2026_REQUIRED_FROM } from "@/types";
import {
...
export default function ApplyForm({ applicationType, mode = "fund", prefill = null, draft = null, isAdmin = false, adminApplicantId = null, adminUser = null, onBack }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (applicationType === "grade" && (gradeDetail.subType === "minor" || gradeDetail.subType === "double")) {
      ...
      // 2026 개편 MD(2027-08~): MD 과목 학점의 절반만 인정 (12학점 중 6학점) → 절반 자동 불인정
      const mdCredits = isOldMd ? 0 : courses.filter((c) => c.mdProgramId).reduce((s, c) => s + (Number(c.credits) || 0), 0);
      const netCredits = courses.reduce((s, c) => s + (Number(c.credits) || 0), 0) - Math.floor(mdCredits / 2);
