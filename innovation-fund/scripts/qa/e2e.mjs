...
async function adminGate(page, pw) {
  ...
  if (!shown) return; // 이미 인증된 세션 — 게이트 없음
  // 게이트가 재렌더/detach되어도 진행하도록 방어적으로 처리
  await gate.fill(pw).catch(() => {});
  await page.getByRole("button", { name: "확인", exact: true }).click().catch(() => {});
  await page.waitForTimeout(1000);
  // 혹시 아직 게이트가 남아있으면 한 번 더 시도
...
await step("D2", "신청 상세 — 보완요청 사유 필수화 차단 → 메모 입력 후 저장", async () => {
  const p = pagePM;
...
await step("F3b", "신청 상세 — 상태 변경 이력(감사 로그) 기록 검증", async () => {
  const p = pageEX;
  // 감사 로그가 서버에 실제로 누적 저장되었는지 API로 확인(검토→승인, 지급→지출완료)
  // pageEX가 F3에서 연 상세 URL에서 신청 id 추출(정확)
  const id = p.url().split("?")[0].replace(/\/$/, "").split("/").pop();
  const probe = await p.evaluate(async (id) => {
    const r = await fetch(`/api/applications/${id}`);
    return { status: r.status, body: await r.text() };
  }, id);
  if (probe.status !== 200) throw new Error(`GET 실패 id=${id} status=${probe.status} body=${probe.body.slice(0, 150)}`);
  const data = JSON.parse(probe.body);
  const hist = Array.isArray(data.statusHistory) ? data.statusHistory : [];
  const hasApprove = hist.some((h) => h.field === "review" && h.to === "approved");
  const hasPay = hist.some((h) => h.field === "payment" && h.to === "completed");
  if (!hasApprove || !hasPay) {
    throw new Error(`감사 로그 미기록 — 항목 ${hist.length}건: ${JSON.stringify(hist).slice(0, 200)}`);
  }
  // UI 카드도 떠 있는지 캡처(있으면 스크롤해 담기)
  await p.reload({ waitUntil: "networkidle" }).catch(() => {});
  const card = p.locator('.card:has-text("상태 변경 이력")').first();
  if (await card.isVisible({ timeout: 8000 }).catch(() => false)) { await card.scrollIntoViewIfNeeded(); await shot(p, "admin-detail-history"); }
});
