...
export async function POST(req: NextRequest) {
  ...
  // 최종 제출: 검토 상태를 '신청완료'로 설정하고, 프로그램 관리자 재검토 단계로 복귀
  if (finalize) { payload.review_status = "received"; payload.review_stage = null; }

  if (id) {
    const { data: existing } = await admin.from("applications").select("applicant_id,is_draft,review_status").eq("id", id).maybeSingle();
    ...
    // 보완요청 상태에서의 재제출은 '보완완료'로 자동 반영 — 관리자가 신규 신청과 구분 가능
    if (finalize && existing.review_status === "supplement") payload.review_status = "supplemented";
