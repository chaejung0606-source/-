...
// 가상학과 학생 기능은 '신청자 정보' 메뉴 탭으로 통합됨. 이 경로는 직접 접근용으로 유지.
// 비밀번호 게이트 — 메뉴 클릭 시에만 재확인(메뉴 안 이동은 통과 유지, lib/pw-gate).
export default function VirtualStudentsPage() {
  const [unlocked, setUnlocked] = useState(false);
  const [gatePw, setGatePw] = useState("");
  const [gateErr, setGateErr] = useState("");
  useEffect(() => { if (isGateUnlocked("/admin/virtual-students")) setUnlocked(true); }, []);
  const tryUnlock = async (e: React.FormEvent) => {
    ...
    if (j.success) { setUnlocked(true); setGatePw(""); unlockGate("/admin/virtual-students"); }
  ...
  if (!unlocked) return (
    <AdminLayout>
      <div className="max-w-sm mx-auto mt-16 card text-center">
        ...
        <p className="text-sm text-gray-500 mb-4">개인정보 보호를 위해 관리자 비밀번호를 입력해주세요.</p>
